...
import httplib2, requests, os, random, string, json
...
def catalog():
    ...
    latest_items = db_session.query(Item).order_by(Item.pub_date.desc()).limit(10)
...
def category(name):
    ...
    return render_template('category.html', categories=categories, category=category, items=category.items)
...
def newCategory():
    ...
    if form.validate_on_submit():
        ...
        if form.name.data.lower() == 'new':
            form.name.errors.append("'new' is a reserved word, and cannot be used as a category name.")
...
def newItem():
    ...
    if form.validate_on_submit():
        # check that name != 'new', which is used for routing
        if form.name.data.lower() == 'new':
            form.name.errors.append("'new' is a reserved word, and cannot be used as an item name.")
